...
export class MatchmakingService {
  private queues: Map<QueueType, Map<string, QueueEntry>> = new Map([
    ['ranked', new Map()],
    ['normal', new Map()],
    ['practice', new Map()],
  ]);
  ...
  private draftManagers: Map<string, DraftManager> = new Map();
  private timeoutTimers: Map<string, NodeJS.Timeout> = new Map();
  private io: Server | null = null;

  constructor() {
    // Initialize queues
    for (const queueType of ['ranked', 'normal', 'practice'] as QueueType[]) {
  ...
  async addToQueue(
    ...
    logger.info(`📥 Player ${username} joined ${data.queueType} queue (MMR: ${user.mmr})`);
  ...
  removeFromQueue(
    ...
    logger.info(`📤 Player ${entry.username} left ${queueType} queue (reason: ${reason})`);
  }

  /**
   * Cancel queue for a specific socket
   */
  cancelQueue(socket: Socket): void {
    const userId = (socket as any).user?.userId;
    const queueType = socket.data.queueType as QueueType;
   ...
   */
  private processQueue(queueType: QueueType): void {
    const queue = this.queues.get(queueType);
    if (!queue || queue.size < CONFIG.TEAM_SIZE) return;
    ...
    if (players.length < CONFIG.TEAM_SIZE * 2) return;
  ...
  private findMatch(
    ...
    if (players.length < CONFIG.TEAM_SIZE * 2) return null;
   ...
   */
  private attemptMatch(players: QueueEntry[], queueType: QueueType): QueueEntry[] | null {
    // Check MMR spread
    ...
    const teamBlue: QueueEntry[] = [];
    const teamRed: QueueEntry[] = [];
   ...
   */
  private calculateTeamBalance(players: QueueEntry[]): number {
   ...
   */
  private groupByRole(players: QueueEntry[]): Map<Role | 'fill', QueueEntry[]> {
    const groups = new Map<Role | 'fill', QueueEntry[]>();
   ...
   */
  private async createMatch(players: QueueEntry[], queueType: QueueType): Promise<void> {
    const teamBlue = players.slice(0, CONFIG.TEAM_SIZE);
    ...
    const draftManager = new DraftManager(ticketId, match.id, {
      blueTeam: teamBlue,
      redTeam: teamRed,
    });
    ...
    logger.info(
      `🎮 Match created: ${match.id} - Blue avg MMR: ${teamBlue.reduce((s, p) => s + p.mmr, 0) / 5}, Red avg MMR: ${teamRed.reduce((s, p) => s + p.mmr, 0) / 5}`
    );
  ...
  private notifyMatchFound(
    ...
    logger.info(`📢 Match found for ${player.username} (${teamSide} team)`);
  }

  /**
   * Broadcast draft action to all players in match
   */
  private broadcastDraftAction(ticketId: string, data: any): void {
    if (!this.io) return;
   ...
   */
  private async handleDraftComplete(ticketId: string, draftState: any): Promise<void> {
    const ticket = this.tickets.get(ticketId);
    const draftManager = this.draftManagers.get(ticketId);

    if (!ticket || !draftManager) return;
    ...
    logger.info(`✅ Draft complete for match ${gameId}`);
   ...
   */
  private async handleDraftTimeout(ticketId: string, data: any): Promise<void> {
    if (!this.io) return;
   ...
   */
  handleDraftAction(ticketId: string, userId: string, action: any): void {
    const draftManager = this.draftManagers.get(ticketId);
   ...
   * Get queue position
   */
  private getQueuePosition(queueType: QueueType): number {
    const queue = this.queues.get(queueType);
    return queue?.size ?? 0;
  }
   ...
   */
  private estimateWaitTime(queueType: QueueType, mmr: number): number {
    const queue = this.queues.get(queueType);
    ...
    const similarMMR = Array.from(queue.values()).filter(
      p => Math.abs(p.mmr - mmr) <= CONFIG.MMR_TOLERANCE
    );
   ...
   */
  getQueueStats(): { queueType: QueueType; count: number }[] {
    const stats: { queueType: QueueType; count: number }[] = [];
   ...
   */
  getTicketForPlayer(userId: string): MatchmakingTicket | null {
  ...
  cancelAllQueues(): void {
    ...
    this.tickets.clear();
    logger.info('🧹 All queues cancelled');
  }
  ...
  async processGameResult(
    ...
    winner: TeamSide,
    duration: number,
    playerStats: { userId: string; kills: number; deaths: number; assists: number; championId: string; role: Role }[]
  ): Promise<void> {
    ...
    if (!match || match.queueType !== 'ranked') return;

    // Calculate MMR changes
    const mmrChanges = await mmrCalculator.calculateMatchMMR(
      playerStats,
      winner,
      duration
    );
    ...
    logger.info(`📊 MMR updated for match ${gameId}: Winner=${winner}`);
  }
}
