/**
 * ItemSystem - Item effects, stats, purchase system
 * Items provide AD, AP, armor, MR, attack speed bonuses
 */
import { v4 as uuid } from 'uuid';
import type { GameEntity } from '@shared/types/game';
...
export interface ItemStats {
  ...
  healPower?: number;
  onHitDamage?: number;
  onHitMagicDamage?: number;
  armorShredOnHit?: number;
}

export interface ItemEffect {
  type: 'onHit' | 'onAttack' | 'onKill' | 'onAssist' | 'onDamage' | 'onHeal' | 'passive' | 'active';
  name: string;
  cooldown?: number;
  duration?: number;
...
export class ItemSystem {
  ...
  private equippedItems: Map<string, EquippedItems> = new Map();
  private activeEffects: Map<string, Map<string, number>> = new Map(); // entityId -> (effectId -> cooldown)
  ...
  purchaseItem(entityId: string, itemId: string): boolean {
    ...
    if (equipment.itemCount >= GAME_CONSTANTS.MAX_ITEMS) return false;

    // Check if can afford
    if (entity.current.gold < item.cost) return false;
  ...
  private applyItemStats(entityId: string, item: Item): void {
    const entity = this.engine.getEntity(entityId);
    if (!entity) return;
    ...
    if (stats.health) entity.stats.health! += stats.health;
    if (stats.mana) entity.stats.mana! += stats.mana;
    if (stats.ad) entity.stats.currentAD += stats.ad;
    if (stats.ap) entity.stats.currentAP += stats.ap;
    if (stats.armor) entity.stats.currentArmor += stats.armor;
    if (stats.mr) entity.stats.currentMR += stats.mr;
    if (stats.attackSpeed) entity.stats.currentAttackSpeed += stats.attackSpeed / 100;
    if (stats.moveSpeed) entity.stats.currentMoveSpeed += stats.moveSpeed;
    if (stats.critChance) entity.stats.critChance += stats.critChance / 100;
    if (stats.critDamage) entity.stats.critMultiplier += stats.critDamage / 100;
    if (stats.lifesteal) entity.stats.lifesteal += stats.lifesteal / 100;
    if (stats.armorPenFlat) entity.stats.armorPenetration += stats.armorPenFlat;
    if (stats.armorPenPercent) entity.stats.armorPenetration += stats.armorPenPercent; // Add to existing percent
    if (stats.magicPenFlat) entity.stats.magicPenetration += stats.magicPenFlat;
    if (stats.magicPenPercent) entity.stats.magicPenetration += stats.magicPenPercent;
    if (stats.cdr) entity.stats.cdr += stats.cdr;
  ...
  private removeItemStats(entityId: string, item: Item): void {
    const entity = this.engine.getEntity(entityId);
    if (!entity) return;
    ...
    if (stats.lifesteal) entity.stats.lifesteal -= stats.lifesteal / 100;
    if (stats.armorPenFlat) entity.stats.armorPenetration -= stats.armorPenFlat;
    if (stats.armorPenPercent) entity.stats.armorPenetration -= stats.armorPenPercent;
    if (stats.magicPenFlat) entity.stats.magicPenetration -= stats.magicPenFlat;
    if (stats.magicPenPercent) entity.stats.magicPenetration -= stats.magicPenPercent;
    if (stats.cdr) entity.stats.cdr -= stats.cdr;
  ...
  getItemStat(entityId: string, statName: string): number {
    ...
    for (const item of equipment.slots) {
      if (!item) continue;
      const value = (item.stats as any)[statName];
   ...
   */
  getItemBonuses(entityId: string): ItemStats {
    const equipment = this.equippedItems.get(entityId);
    if (!equipment) return {};
    ...
    for (const item of equipment.slots) {
      ...
      for (const [key, value] of Object.entries(item.stats)) {
        (bonuses as any)[key] = ((bonuses as any)[key] || 0) + value;
      }
    }
   ...
   */
  processOnHit(attackerId: string, targetId: string): void {
  ...
  private processOnHitEffect(attacker: GameEntity, target: GameEntity, item: Item, effect: ItemEffect): void {
    ...
    // Check cooldown
    if (effect.cooldown && now - lastUsed < effect.cooldown) return;
   ...
   */
  useActiveItem(entityId: string, itemSlot: number): boolean {
    const equipment = this.equippedItems.get(entityId);
    if (!equipment) return false;
    ...
    if (this.engine.state.time - lastUsed < cooldown) return false;
    ...
    switch (activeEffect.name) {
      ...
      case 'Intervention': // Redemption
        ...
        for (const ally of Object.values(this.engine.state.entities)) {
          if (ally.team !== entity.team) continue;
  ...
  getEquippedItems(entityId: string): Item[] {
    const equipment = this.equippedItems.get(entityId);
    if (!equipment) return [];
    return equipment.slots.filter((s): s is Item => s !== null);
  }

  canAffordItem(entityId: string, itemId: string): boolean {
    const entity = this.engine.getEntity(entityId);
    if (!entity) return false;
    const item = ITEM_DATABASE[itemId];
  ...
  getItemCost(itemId: string): number {
    return ITEM_DATABASE[itemId]?.cost || 0;
  }
