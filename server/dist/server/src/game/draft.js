...
// DRAFT MANAGER
// ==========================================
class DraftManager extends events_1.EventEmitter {
    ticketId;
    gameId;
    blueTeam;
    redTeam;
    ...
    unavailableChampions = new Set();
    // Current acting player (for picks, any player can pick)
    currentActor = null;
    constructor(ticketId, gameId, teams) {
        super();
        ...
        logger_1.logger.info(`🎯 DraftManager initialized for game ${gameId}`);
    }
    /**
     * Start the draft process
     */
    start() {
        logger_1.logger.info(`🚀 Starting draft for game ${this.gameId}`);
        this.nextPhase();
    }
    /**
     ...
     */
    processAction(championId, team, type, playerId, forced = false) {
        ...
        logger_1.logger.info(`🎯 ${team.toUpperCase()} ${type}s ${championId} (forced: ${forced})`);
    }
    /**
    ...
    handlePhaseTimeout() {
        const currentPhase = DRAFT_SEQUENCE[this.currentPhaseIndex];
        const phaseInfo = PHASE_TO_ACTION[currentPhase];
        logger_1.logger.warn(`⏱️ Draft phase ${currentPhase} timed out`);
        // Get random available champion
        const available = this.getAvailableChampions();
        if (available.length === 0) {
            logger_1.logger.error('No available champions for random selection!');
            this.currentPhaseIndex++;
            this.nextPhase();
            return;
        }
    ...
    async completeDraft() {
        ...
        logger_1.logger.info(`✅ Draft complete for game ${this.gameId}`);
    ...
    async createMatchPlayers() {
        ...
        if (!match) {
            logger_1.logger.error(`Match not found: ${this.gameId}`);
            return;
        }
        ...
        for (let i = 0; i < this.state.picks.blue.length; i++) {
            ...
            if (queueEntry) {
                await database_1.prisma.matchPlayer.create({
                    data: {
                        matchId: match.id,
    ...
    cancel(reason) {
        ...
        logger_1.logger.info(`❌ Draft cancelled for game ${this.gameId}: ${reason}`);
