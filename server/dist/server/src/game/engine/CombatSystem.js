...
class CombatSystem {
     ...
     */
    calculatePhysicalDamage(baseDamage, target) {
     ...
     */
    calculateMagicDamage(baseDamage, target) {
     ...
     */
    calculateTrueDamage(baseDamage) {
        return Math.floor(baseDamage);
    }
    /**
     * Calculate execute damage based on target's missing health
     */
    calculateExecuteDamage(baseDamage, target, executeThreshold = 0.5) {
     ...
     */
    calculateCritDamage(baseDamage, critMult) {
     ...
     */
    attemptAutoAttack(attackerId, targetId) {
        const attacker = this.engine.getEntity(attackerId);
        const target = this.engine.getEntity(targetId);
        if (!attacker || !target)
            return false;
        if (this.engine.isDead(attacker) || this.engine.isDead(target))
            return false;
     ...
     */
    findAttackTarget(entityId) {
        const entity = this.engine.getEntity(entityId);
        if (!entity)
            return null;
        const range = entity.stats.currentRange;
        const enemies = this.engine.getLivingEntities().filter(e => e.team !== entity.team && this.physics.distance(entity.position, e.position) <= range);
        if (enemies.length === 0)
            return null;
    ...
    registerOnHitEffect(itemId, effect) {
    ...
    applyOnAttackEffects(attacker, target) {
    ...
    // BUFFS & DEBUFFS
    // ==========================================
    applyBuff(targetId, stat, value, duration) {
    ...
    applyDebuff(targetId, stat, value, duration) {
        const entity = this.engine.getEntity(targetId);
        if (!entity)
            return;
    ...
    // HEALING & SHIELDING
    // ==========================================
    applyHealing(sourceId, targetId, amount) {
        const target = this.engine.getEntity(targetId);
        if (!target)
            return;
    ...
    applyShield(targetId, amount, duration) {
        const target = this.engine.getEntity(targetId);
        if (!target)
            return;
        target.buffs.push({
     ...
     */
    applyAOEDamage(sourceId, center, radius, baseDamage, damageType) {
    ...
    // ==========================================
    applyDOT(sourceId, targetId, damage, duration, tickRate = 1) {
        const target = this.engine.getEntity(targetId);
        if (!target)
            return;
        ...
        const dotId = (0, uuid_1.v4)();
        const applyTick = () => {
            const currentTarget = this.engine.getEntity(targetId);
            if (!currentTarget || this.engine.isDead(currentTarget))
                return;
    ...
    // CROWD CONTROL
    // ==========================================
    applyStun(targetId, duration, sourceId) {
        const entity = this.engine.getEntity(targetId);
        if (!entity)
            return;
        entity.states.push({
    ...
    applyRoot(targetId, duration, sourceId) {
        const entity = this.engine.getEntity(targetId);
        if (!entity)
            return;
        entity.states.push({
    ...
    applySlow(targetId, amount, duration, sourceId) {
        const entity = this.engine.getEntity(targetId);
        if (!entity)
            return;
    ...
    applySilence(targetId, duration, sourceId) {
        const entity = this.engine.getEntity(targetId);
        if (!entity)
            return;
        entity.states.push({
    ...
    applyFear(targetId, duration, sourceId) {
        const entity = this.engine.getEntity(targetId);
        if (!entity)
            return;
        entity.states.push({
    ...
    reverseStatModification(entity, stat, value) {
