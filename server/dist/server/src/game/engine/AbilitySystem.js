...
class AbilitySystem {
    engine;
    cooldowns = new Map();
    cooldownRefunds = new Map();
    abilityLevels = new Map();
    passiveTimers = new Map();
    channeledAbilities = new Map();
    // Cache for champion data
    championAbilities = new Map();
    constructor(engine) {
        this.engine = engine;
        this.initializeChampionAbilities();
    }
    initializeChampionAbilities() {
        for (const [championId, champion] of Object.entries(champions_1.CHAMPION_DATA)) {
     ...
     */
    executeAbility(caster, ability, def, level) {
    ...
    executeLineAbility(caster, ability, def, level, effects) {
    ...
    executeConeAbility(caster, ability, def, level, effects) {
        const targetPos = ability.targetPosition || caster.position;
        const range = this.getAbilityRange(def, level);
        const angle = this.engine.physics.angle(caster.position, targetPos);
    ...
    executeAreaAbility(caster, ability, def, level, effects) {
    ...
    executeTargetedAbility(caster, ability, def, level, effects) {
    ...
    executeSelfAbility(caster, _ability, def, level, effects) {
    ...
    executeGlobalAbility(caster, ability, def, level, effects) {
    ...
    executeAutoAbility(caster, ability, def, level, effects) {
    ...
    // EFFECT APPLICATION
    // ==========================================
    applyEffects(caster, target, effects, ability) {
    ...
    applyDamageEffect(caster, target, effect, ability) {
    ...
    applyShieldEffect(caster, target, effect) {
    ...
    applyBuffEffect(caster, target, effect) {
        const value = effect.value || 0;
    ...
    applyCharmEffect(caster, target, effect) {
        const duration = effect.duration || 1;
    ...
    // ==========================================
    calculateAbilityDamage(effects, caster) {
    ...
    reduceCooldown(entityId, abilityKey, amount) {
        const entityCds = this.cooldowns.get(entityId);
        if (!entityCds)
            return;
        const cd = entityCds.get(abilityKey);
    ...
    // ABILITY LEVELING
    // ==========================================
    getAbilityLevel(entityId, abilityKey) {
    ...
    levelUp(entityId, abilityKey) {
        ...
        if (abilityKey === 'R' && entity.current.level < 6)
            return false;
        if (abilityKey === 'R' && currentLevel >= 2 && entity.current.level < 11)
            return false;
        if (abilityKey === 'R' && currentLevel >= 3 && entity.current.level < 16)
            return false;
    ...
    initializeAbilityLevels(entityId) {
    ...
    processPassives(entity, dt) {
        ...
        if (entity.current.health < entity.current.maxHealth) {
            entity.current.health = Math.min(entity.current.maxHealth, entity.current.health + healthRegen * dt);
        }
        // Mana regen
        const manaRegen = 8; // Base mana regen per second
        if (entity.current.mana < entity.current.maxMana) {
            entity.current.mana = Math.min(entity.current.maxMana, entity.current.mana + manaRegen * dt);
        }
        // Process champion-specific passives
        this.processChampionPassive(entity, dt);
    }
    processChampionPassive(entity, dt) {
    ...
    // CHANNELING
    // ==========================================
    startChannel(casterId, ability, duration) {
        const caster = this.engine.getEntity(casterId);
        if (!caster)
            return false;
        if (this.channeledAbilities.has(casterId)) {
    ...
    getAbilityDefinition(entityId, key) {
        ...
        const championId = entity.championId || 'lux';
