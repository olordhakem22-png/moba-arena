...
exports.mmrCalculator = exports.MMRCaclulatorWrapper = exports.MMRCalculator = void 0;
/**
...
class MMRCalculator {
     ...
     */
    calculateExpectedScore(playerMMR, opponentMMR) {
     ...
     */
    calculateMMRChange(playerMMR, teamAverageMMR, opponentAverageMMR, won, kdaModifier = 0, streakMultiplier = 1) {
     ...
     */
    calculateTeamBalanceMultiplier(teamMMRSpread) {
        if (teamMMRSpread <= 50)
            return 1.05;
        if (teamMMRSpread <= 100)
            return 1.0;
        if (teamMMRSpread <= 150)
            return 0.95;
        return 0.9;
    }
    /**
     ...
     */
    calculateKDAModifier(kills, deaths, assists) {
        if (deaths === 0) {
            // Perfect game bonus
        ...
        if (kda >= 4)
            return CONFIG.KDA_BONUS_MAX;
        if (kda >= 3)
            return CONFIG.KDA_BONUS_MAX * 0.5;
        // Poor performance (KDA < 0.5) = penalty
        if (kda <= 0.3)
            return CONFIG.KDA_PENALTY_MAX;
        if (kda <= 0.5)
            return CONFIG.KDA_PENALTY_MAX * 0.5;
        return 0;
    }
    /**
    ...
    getRankFromMMR(mmr) {
        ...
        if (mmr >= thresholds.CHALLENGER)
            return 'CHALLENGER';
        if (mmr >= thresholds.GRANDMASTER)
            return 'GRANDMASTER';
        if (mmr >= thresholds.MASTER)
            return 'MASTER';
        if (mmr >= thresholds.DIAMOND)
            return 'DIAMOND';
        if (mmr >= thresholds.PLATINUM)
            return 'PLATINUM';
        if (mmr >= thresholds.GOLD)
            return 'GOLD';
        if (mmr >= thresholds.SILVER)
            return 'SILVER';
        if (mmr >= thresholds.BRONZE)
            return 'BRONZE';
        return 'UNRANKED';
    }
    /**
     ...
     */
    calculateLP(mmrChange, currentRank) {
     ...
     */
    calculatePositionInTier(mmr, rank) {
        if (rank === 'UNRANKED')
            return 1000;
        const rankStart = CONFIG.RANK_THRESHOLDS[rank];
        const rankEnd = rank === 'CHALLENGER'
...
exports.MMRCalculator = MMRCalculator;
// ==========================================
// HELPER FUNCTIONS
// ==========================================
...
class MMRCaclulatorWrapper {
     ...
     */
    async calculateMatchMMR(playerStats, winner, duration) {
        const changes = {};
        // Get all player MMRs
        ...
        for (const player of playerStats) {
            ...
            // Calculate KDA modifier
            const kdaModifier = this.calculator.calculateKDAModifier(player.kills, player.deaths, player.assists);
            // Get streak multiplier
            const streakMultiplier = await this.getStreakMultiplier(player.userId, won);
            // Calculate MMR change
            const mmrChange = this.calculator.calculateMMRChange(playerMMR, teamAverage, opponentAverage, won, kdaModifier, streakMultiplier);
            // Calculate new MMR
            const newMMR = Math.max(CONFIG.DEFAULT_NEW_PLAYER_MMR / 2, Math.min(CONFIG.RANK_THRESHOLDS.CHALLENGER + 200, playerMMR + mmrChange));
        ...
        logger_1.logger.info(`📊 MMR calculated for ${playerStats.length} players - Winner: ${winner}`);
        return changes;
    }
    /**
     * Get average MMR for a team
     */
    getAverageMMR(players, userMap) {
        let total = 0;
        let count = 0;
     ...
     */
    async getStreakMultiplier(userId, won) {
        const user = await database_1.prisma.user.findUnique({
            where: { id: userId },
...
exports.MMRCaclulatorWrapper = MMRCaclulatorWrapper;
exports.mmrCalculator = new MMRCaclulatorWrapper();
