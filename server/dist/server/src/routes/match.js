 ...
 */
exports.matchRoutes.get('/stats', auth_1.authenticate, async (req, res, next) => {
    try {
        const players = await database_1.prisma.matchPlayer.findMany({
            where: { userId: req.user.userId },
            include: {
 ...
 */
exports.matchRoutes.get('/:id/replay', auth_1.authenticate, async (req, res, next) => {
    try {
        const match = await database_1.prisma.match.findUnique({
            where: { id: req.params.id },
            include: {
 ...
 */
exports.matchRoutes.get('/ranked/history', auth_1.authenticate, async (req, res, next) => {
    try {
        const players = await database_1.prisma.matchPlayer.findMany({
            where: {
 ...
 */
exports.matchRoutes.get('/ranked/stats', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        // Get ranked games
        const rankedGames = await database_1.prisma.matchPlayer.count({
            where: {
                userId: req.user.userId,
                match: { queueType: 'ranked', status: 'ENDED' },
            },
        ...
        const recentGames = await database_1.prisma.matchPlayer.findMany({
            where: {
                userId: req.user.userId,
                match: { queueType: 'ranked', status: 'ENDED' },
            },
        ...
        const recentResults = recentGames.map(p => p.match.winner === p.team ? 'win' : 'loss');
 ...
 */
exports.matchRoutes.get('/ranked/leaderboard', auth_1.authenticate, async (req, res, next) => {
    try {
 ...
 */
exports.matchRoutes.post('/:id/report', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        if (match.queueType === 'ranked') {
            const { matchmakingService } = await Promise.resolve().then(() => __importStar(require('../game/matchmaking')));
            await matchmakingService.processGameResult(match.gameId, winner, duration || 0, playerStats);
        }
        // Update match record
        ...
        for (const stat of playerStats) {
            await database_1.prisma.matchPlayer.updateMany({
                where: {
 ...
 * Leave a custom game
 */
exports.matchRoutes.post('/:id/leave', auth_1.authenticate, async (req, res, next) => {
    try {
        await database_1.prisma.matchPlayer.deleteMany({
            where: {
                matchId: req.params.id,
                userId: req.user.userId,
            },
 ...
 */
exports.matchRoutes.get('/custom/lobbies', auth_1.authenticate, async (req, res, next) => {
    try {
        const lobbies = await database_1.prisma.match.findMany({
            where: {
                queueType: 'custom',
                status: 'PENDING',
            },
